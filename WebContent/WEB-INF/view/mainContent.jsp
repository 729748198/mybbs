	...
	<div class="row">
		<div class="col-xs-2 text-right">
			<c:choose>
				...
				<c:when test="${mainType eq 'algorithm'}">
					<img alt="" src="<%=basePath %>secondImage/language_length10.png">
				</c:when>
  		...
  		<c:choose>
  			<c:when test="${not empty main }">
  				<c:forEach items="${main }" var="item" varStatus="vs">
  					<tr>
  						<td>
  							<c:if test="${banzhu[0]['wxname'] eq sessionScope.loginEntity.wxname }">
	  							<c:if test="${item.main_recommend eq 1 }">
		  								...
		  								<c:if test="${sessionScope.loginFlag}">
		  									<a id="quxiao" style="color: green" href="javascript:void(0)" onclick="setjinghua('${item.main_id}','qx')">[取消精华]</a>
		  							...
		  							<c:if test="${item.main_recommend ne 1 }">
		  								...
		  								<c:if test="${sessionScope.loginFlag}">
		  								<a  id="shezhi"  style="color: red" href="javascript:void(0)" onclick="setjinghua('${item.main_id}','sz')">[设置精华]</a>
  						...
  						<c:if test="${ banzhu[0]['wxname'] != sessionScope.loginEntity.wxname}">
	  							<c:if test="${item.main_recommend eq 1 }">
		  								<a href="<%=basePath%>secondContent?mainId=${item.main_id}" ><img src="<%=basePath %>image/pin_1.gif" id="${item.main_id}img" />[精品]&nbsp;&nbsp; ${item.main_title }</a>
